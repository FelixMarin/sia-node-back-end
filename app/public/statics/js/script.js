...
const traerProductos = function(producto, ordenar, strEmpresas) {
    ...
    let bufferProducts = [];
    strEmpresas.split(',').forEach((elem, index) => {
        
        $.ajax({
            type: "GET",
            url: "/search?country=101&category=101&sort="+ ordenar +"&product="+producto + "&pipedcompanies=" + elem,
            dataType: "text",
            timeout: 1000000,
            beforeSend: function(xhr) {
                ...
                if(Object.is(data,window.localStorage.getItem('data'))) {
                    ...
                    if(ordenar == 1) {
                        bufferProducts = bufferProducts.sort((a,b) => (a.precio.match(regex)[0] > b.precio.match(regex)[0]) ? 1 : ((b.precio.match(regex)[0] > a.precio.match(regex)[0]) ? -1 : 0))
                    } else if(ordenar == 2) {
                        bufferProducts = bufferProducts.sort((a,b) => (a.precioKilo.match(regex)[0] > b.precioKilo.match(regex)[0]) ? 1 : ((b.precioKilo.match(regex)[0] > a.precioKilo.match(regex)[0]) ? -1 : 0))
                    }

                    componerCartas(bufferProducts);
...
const componerCartas = function(bufferProducts) {
    let estructuraHTML = '';
    let contador = 0;
    
    if(bufferProducts.includes("[]")) { 
    ...
    try {
        $('#productos').html('');
        let datosJSON = jQuery.parseJSON(JSON.stringify(bufferProducts));
        ...
        datosJSON.forEach(element => {           
            ...
            estructuraHTML += colorIdentificador(conuter++);
            estructuraHTML += '</h4>'
